...
#include "SVF/SVF-LLVM/BasicTypes.h"
#include "SVF/Util/CppUtil.h"
...
public:
    ...
    inline const Instruction* getInstruction(const SVFInstruction* inst) const
    {
      for(auto& it : LLVMInst2SVFInst) {
	if(it.second == inst){
	  return it.first;
	}
      }
